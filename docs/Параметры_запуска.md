...
Java параметры запуска сервера приложений задаются в его команде запуска (например для [ручной](Выполнение_ручная.md#command-broken) или [автоматической](Выполнение_авто.md#settings-broken) установки) :
...
|Пользовательские (начинаются на D)|<div class="content-wrapper"><br/><p>-Dlsfusion.server.lightstart</p><br/></div>|boolean|<p>Режим "облегченного" запуска (как правило используется при разработке). В этом режиме сервер не выполняет операции синхронизации метаданных, создания форм настройки [политики безопасности](Политика_безопасности.md) и т.п., соответственно уменьшается время запуска и объем потребляемой памяти при запуске.</p><br/><p>В [IDE](IDE.md) регулируется галочкой в [lsFusion server конфигурации](IDE.md#configuration) (по умолчанию включена).</p>|false|
|<div class="content-wrapper"><br/><p>-Dlsfusion.server.devmode</p><br/></div>|boolean|<p>Режим разработки. В этом режиме:</p><br/><ul><br/><li>Не запускаются системные задачи (чтобы не мешать отладчику)</li><br/><li>Включается возможность редактирования [дизайна отчетов](Дизайн_отчетов.md) в [интерактивном печатном](В_печатном_представлении_PRINT.md#interactive) представлении</li><br/><li>Включается анонимный доступ к API и UI ([системные параметры](Параметры_работы.md) enableAPI, enableUI). Кроме того в этом режиме анонимный доступ идет под админом, а не анонимным пользователем</li><br/><li>Клиент автоматически переподключается при потере связи</li><br/><li>Выключается кэш чтения отчетов из ресурсов</li><br/></ul><br/><p>В [IDE](IDE.md) автоматически включается при запуске [lsFusion server конфигурации](IDE.md#configuration).</p>|false|
...
По умолчанию предполагается, что файлы параметров запуска conf/settings.properties и lsfusion.properties находятся в папке запуска сервера приложений. Впрочем при [автоматической установке](Выполнение_авто.md) под Linux для этих файлов (как и для папок [логов](Журналы_и_логи.md#logs))  автоматически создаются symlink'и на [другие файлы](Выполнение_авто.md#settings-broken), расположение которых лучше соответствует идеологии Linux.

### Веб-сервер (Client)

#### Java {#appjava}

Java параметры запуска веб-сервера задаются в команде запуска Tomcat, на котором, в свою очередь, запускается этот веб-сервер (например для [автоматической](Выполнение_авто.md#webapp-broken) установки). 
...
-   В параметрах [контекста](http://tomcat.apache.org/tomcat-7.0-doc/config/context.html#Defining_a_context) веб-приложения:
    ...
    -   в Tomcat в файле $CATALINA\_BASE/conf/\[enginename\]/\[hostname\]/\[contextpath\].xml, тег Context, тег Parameter, где:
        -   $CATALINA\_BASE$ - папка, в которую установлен Tomcat (например, в [автоматической](Выполнение_авто.md#settings-broken) установке, эта папка равна $INSTALL\_DIR/Client)
        -   \[contextpath\] - контекстный путь веб-приложения (например, в [автоматической](Выполнение_авто.md#settings-broken) установке, по умолчанию это имя пустое, что в Tomcat'е эквивалентно имени ROOT, в [ручной ](Выполнение_ручная.md#tomcat-broken)- зависит от имени war-файла), 
        -   \[enginename\] и \[hostname\] - имена механизма реализации tomcat и компьютера веб-сервера (например в [автоматической](Выполнение_авто.md#settings-broken) установке, эти имена равны catalina и localhost соответственно)
