...
Оператор **IMPORT **- создание [действия](Действия.md), импортирующего данные из заданного файла в [заданные свойства (параметры)](Импорт_данных_IMPORT.md) или, в общем случае, в [заданную форму](В_структурированном_представлении_EXPORT_IMPORT.md#Вструктурированномпредставлении(EXPORT,IMPORT-broken)-importForm).
...
Классы [импортируемых](Импорт_данных_IMPORT.md#Импортданных(IMPORT-broken)-objects) объектов. Задаются [идентификаторами класса](Идентификаторы.md#classid-broken). K <= 1. По умолчанию считается, что импортируется один объект класса **INTEGER**.
